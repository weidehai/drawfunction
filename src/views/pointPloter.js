...
export const PonitPloter = (function() {
  ...
  PonitPloter.prototype.polar = function() {
    ...
    if (this._x <= this.polarRange[1] && !this.stopPlot) {
      this._x = reliableFloatAdd(this._x, this.polarStep);
      this.x = degToarc(this._x);
      if (!this.enableAnimation || this.animationSpeed <= 0 || this.canvas.moving) {
        this.polar();
      } else {
  ...
  PonitPloter.prototype.cartesian = function() {
    ...
    if (isNull(this.x)) {
      this.x = (this.cartesianRange[0] - this.canvas.originPointX) / this.zoom;
    }
    try {
      eval(this.expression);
    } catch (e) {
    ...
    if (this.realx <= this.cartesianRange[1] && !this.stopPlot) {
  ...
  function count(){
    ...
    count = function (){
      ...
      timeOutTask = setTimeout(()=>{alert(c);c=0},400)
    }
    count()
  }
  ...
  PonitPloter.prototype.ensureStopAllPlot = (function(){
    let timeOutTaskId = null;
    function main(cb) {
      if(timeOutTaskId) {clearTimeout(timeOutTaskId)}
      if (this.stopPlot) return cb();
      this.stopPlot = true;
      timeOutTaskId = setTimeout(() => {
        this.stopPlot = false;
        cb();
